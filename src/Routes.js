...
const UpdateTextPost = lazy(() => import("../Pages/Publication/Article/UpdateTextPost"));
...
const QuestionInterviews = lazy(() => import("../Pages/Question/QuestionInterviews"));
...
const Routes = () => {
	...
	return (
		<Box height="100%" className="route" width="100%">
          ...
          <Route
            path="/update/:type/:id"
            element={<Suspense fallback={<Loader />}>{currentUser ? <UpdateTextPost /> : <Login />}</Suspense>}
          />
          <Route
            path="/login"
